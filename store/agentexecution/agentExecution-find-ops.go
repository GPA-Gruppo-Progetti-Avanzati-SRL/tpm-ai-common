...
func FindByPk(collection *mongo.Collection, objectId string, mustFind bool, findOptions *options.FindOneOptionsBuilder) (*AgentExecution, bool, error) {
	// @tpm-schematics:end-region("find-by-pk-signature-section")
	const semLogContext = "agent-execution::find-by-pk"
...
func FindFirst(collection *mongo.Collection, f *Filter, findOptions *options.FindOptionsBuilder) (*AgentExecution, error) {
	const semLogContext = "agent-execution::find-first"
	fd := f.Build()
...
func Find(collection *mongo.Collection, f *Filter, withCount bool, findOptions *options.FindOptionsBuilder) (QueryResult, error) {
	const semLogContext = "agent-execution::find"
	fd := f.Build()
...
type AgentsToProcessOptions struct {
	LimitCount         int
	LimitWeight        int
	MaxLimitWeight     int
...
func FindPromptsToProcess(coll *mongo.Collection, options AgentsToProcessOptions) ([]AgentExecution, bool, error) {
	const semLogContext = semLogPackageContext + "find-agents-to-process"
...
func FindReadyPrompts(coll *mongo.Collection) ([]AgentExecution, int, error) {
	const semLogContext = semLogPackageContext + "find-ready-agents"
